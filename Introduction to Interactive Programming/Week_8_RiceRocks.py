...
def click(pos):
    global in_game, lives, score
    center = [WIDTH / 2, HEIGHT / 2]
    size = splash_info.get_size()
    inwidth = (center[0] - size[0] / 2) < pos[0] < (center[0] + size[0] / 2)
    inheight = (center[1] - size[1] / 2) < pos[1] < (center[1] + size[1] / 2)
    if (not in_game) and inwidth and inheight:
        in_game = True
        score = 0
        lives = 3
        timer.start()
...
def draw(canvas):
    global time, in_game, rock_group, lives, score, in_game
    ...
    # process ship and asteroid collision
    ...
    for rock in rock_group:
            rock.vel[0] = rock.vel[0] + (rock.vel[0] * score * 0.0001)
            rock.vel[1] = rock.vel[1] + (rock.vel[1] * score * 0.0001)
